...
const AboutSection = () => {
  ...
  return (
    <section className="py-24 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 right-0 w-72 h-72 bg-emerald-500/8 rounded-full blur-3xl animate-morph"></div>
      <div className="absolute bottom-1/4 left-0 w-64 h-64 bg-teal-500/10 rounded-full blur-3xl animate-float"></div>
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-10">
          {aboutItems.map((item, index) => (
            ...
            >
              <h3 className="text-2xl font-bold text-teal-400 mb-6 group-hover:text-emerald-300 transition-colors duration-300">
                {item.title}
              </h3>
              ...
              {/* Decorative element - updated to royal green colors */}
              <div className="absolute top-4 right-4 w-3 h-3 bg-emerald-400/30 rounded-full animate-pulse opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            </div>
          ))}
        </div>
      </div>

      {/* Enhanced parallax wave separator - updated to royal green colors */}
      <div className="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-r from-emerald-600/15 to-teal-600/15 opacity-60 blur-2xl rounded-t-full"></div>
    </section>
  );
};
